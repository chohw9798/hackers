...
*이전 개인정보취급방침 보기(2014년 2월 1일부터 시행된 방침) 
			...
			<div class="all-agree-box">
				<label class="input-sp">
					<input type="checkbox" onclick="allAgree();"/>
					<span class="input-txt">상위 이용약관 및 개인정보 취급방침에 모두 동의합니다.</span>
				</label>
			</div>

			<div class="box-btn">
				<a href="#" class="btn-l" onclick="nextStep();">다음단계 (휴대폰인증)</a>
			</div>
...
<script type="text/javascript">
    function allAgree() {
        if($(".all-agree-box input:checkbox:checked").length==1){
            $(".mt10 input:checkbox").prop('checked', true);
        }
    }

    function nextStep() {
        if($(".mt10 input:checkbox:checked").length==2){
            location.href="/member/index.php?mode=step_02";
        }
